...
//보드 관련된 모든 처리를 다하는 로직이라서,
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("/MemberListAction.do")) {// ��ϵ� �� ��� ������ ����ϱ�
			 ...
			 */
		} else if (command.equals("/MemberWriteForm.do")) { // 글쓰기 폼
			requestLoginName(request);
			RequestDispatcher rd = request.getRequestDispatcher("./member/addMember2.jsp");
			rd.forward(request, response);
		} else if (command.equals("/MemberWriteAction.do")) {// 글쓰기 폼에서 입력 후 처리하는 로직.
			// 여기서 글쓰기작성시 필요한 로직
			// 여기안에 이미지를 등록하는 메서드를 추가 할 예정.
			requestMemberWrite(request);

			RequestDispatcher rd = request.getRequestDispatcher("/MemberListAction.do");
			rd.forward(request, response);
		} else if (command.equals("/MemberViewAction.do")) {// ���õ� �� �� ������ ��������
			requestMemberView(request);
			RequestDispatcher rd = request.getRequestDispatcher("/MemberView.do");
			rd.forward(request, response);
		} else if (command.equals("/MemberView.do")) { // �� �� ������ ����ϱ�
			RequestDispatcher rd = request.getRequestDispatcher("./member/addMember2.jsp");
			rd.forward(request, response);
		} else if (command.equals("/MemberUpdateAction.do")) { // ���õ� ���� ��ȸ�� �����ϱ�
			requestMemberUpdate(request);
			RequestDispatcher rd = request.getRequestDispatcher("/MemberListAction.do");
			rd.forward(request, response);
		} else if (command.equals("/MemberDeleteAction.do")) { // ���õ� �� �����ϱ�
	...
	/*
	 * // 해다 게시판 목록에 보여주는 비지니스 로직. public void requestBoardList(HttpServletRequest
	 * request) {
	 * 
	 * // 임시로 , 게시판 목록 화면에 출력하기 위해서, 해당 정보들의 변수를 선언 및 재할당. // 만약. 이 정보를 계속 사용하겠다고
	 * 하면, 위에서 전역 또는 선언만 하고 // 재할당해서 이용 가능. int listCount =
	 * MemberController.LISTCOUNT; String RequestURI = request.getRequestURI();
	 * String contextPath = request.getContextPath(); String command =
	 * RequestURI.substring(contextPath.length());
	 * 
	 * // 게시판에서 해당 DB에 접근을 하기위한 sql 문장이 모아져있다. // 싱글톤 패턴으로 하나의 객체를 이용을 하고 있음. //
	 * 게시판에 글쓰기, 수정하기, 삭제하기, 리스트 가져오기 등 여러 메서드들이 // dao 라는 객체에 담아져 있다. BoardDAO dao
	 * = BoardDAO.getInstance(); // 컬렉션: 게시판의 글들을 담아두는 역할. // 게시판에 하나의 글들은 각각
	 * BoardDTO 타입의 객체입니다. // boardlist -> 게시판의 각각의 게시글을 담아두었다. 담아 둘 예정. // 디비에
	 * 연결해서, 해당 게시글 목록들을 받아둘 임시 저장 매체로서 사용. List<BoardDTO> boardlist = new
	 * ArrayList<BoardDTO>();
	 * 
	 * int pageNum = 1; // 목록 게시판 보여줄 갯수 5개. int limit = LISTCOUNT;
	 * 
	 * // 만약, 내장객체 request에 담겨진 페이지 정보가 null 아니면. // 해당 페이지 정보를 문자열 -> 정수로 변환하겠다. if
	 * (request.getParameter("pageNum") != null) pageNum =
	 * Integer.parseInt(request.getParameter("pageNum")); // items 는 게시판 화면 하단에 검색하는
	 * 창에서, 본문 검색, 글쓴이 검색, 작성자 검색 등. 조건. String items =
	 * request.getParameter("items"); // text 해당 검색하기위한 검색어. String text =
	 * request.getParameter("text");
	 * 
	 * // dao 게시판에 관련된 메서드들이 다 있고, 전달 할 때, 해당 검색어 항목, 검색할 내용등을 같이 전달. // 디비에 연결해서,
	 * 해당 게시글의 모든 갯수를 가져오는 역할. int total_record = dao.getListCount(items, text);
	 * 
	 * // 실제적인 페이징 처리가 된 결과를 담을 컬렉션이라 보면 됩니다. boardlist = dao.getBoardList(pageNum,
	 * limit, items, text);
	 * 
	 * // total_page 선언만 실제 페이지 계산의 아래에 있음. int total_page;
	 * 
	 * if (total_record % limit == 0) { total_page = total_record / limit;
	 * Math.floor(total_page); } else { total_page = total_record / limit; // 11/5 =
	 * 2.2 double total_page_test = Math.floor(total_page); // 2.2 -> 2 , 내림.
	 * System.out.println("total_page_test의 값 한번 찍어보기." + total_page_test);
	 * total_page = total_page + 1; // 2 + 1 -> 3 } // 해당 RequestURI, contextPath,
	 * command 를 해당 뷰에 데이터를 전달함. // 해당 뷰에서, 키이름으로 해당 값을 불러와서 사용할 예정.
	 * request.setAttribute("RequestURI", RequestURI);
	 * request.setAttribute("contextPath", contextPath);
	 * request.setAttribute("command", command);
	 * 
	 * request.setAttribute("listCount", listCount); request.setAttribute("pageNum",
	 * pageNum); request.setAttribute("total_page", total_page);
	...
	public void requestMemberWrite(HttpServletRequest request) {
...
//			ArrayList<FileImageDTO> fileLists = new ArrayList<FileImageDTO>();
			...
			member.setM_num(multi.getParameter(""));
			...
			member.setBirth(multi.getParameter("birthyy"));
			member.setMail(multi.getParameter("mail1"));
...
//			member.setIp(request.getRemoteAddr());
			 ...
			 * 예)4024bf24-4db3-458b-83b5-23399c8f4a72_bread2 . jpg UUID uuid =
			 * UUID.randomUUID(); // 반복문으로 해당파일을 하나씩 담기 위한 임시 객체. FileImageDTO fileDTO2 =
			 * new FileImageDTO();
			 * 
			 * // fname -> file1, file2, file3 .. //파일을 첨부하는 뷰에서 file1, file2, file3 name
			 * 해당하는 부분. String fname = (String) files.nextElement();
			 * 
			 * // System.out.println("fname" + fname); //원본 파일명 , 해당 name의 실제 파일명. String
			 * fileName = multi.getFilesystemName(fname); // 변경된 파일명 : 아래 형식으로 파일명 중복을 방지.
			 * // fileName : 실제 이미지 파일 명 : 라바1.jfif //
			 * 171f45c0-38fa-42fd-bd4c-63cb8c4847a1_라바1.jfif String uploadFileName =
			 * uuid.toString() + "_" + fileName; fileDTO2.setFileName(uploadFileName);
			 * fileDTO2.setRegist_day(regist_day); fileDTO2.setNum(boardNum); // 받아온 이미지를 임시
			 * 객체인 fileDTO2 에담아서, 여러 객체를 담을 컬렉션에 담는 작업. fileLists.add(fileDTO2); // 업로드된
			 * 파일명을 변경하는 작업. // MultipartRequest 를 사용할 경우 파일 이름을 변경 하여 업로드 할 수 없다. //이유는
			 * multi 생성시 바로 업로드. //그래서 업로드 후에 파일명을 변경 하는 방법.
			 * 
			 * // MultipartRequest 특징 // 해당 객체를 생성하는 순간, 저장 경로에 해당 파일명으로 바로 생성됨. // 그래서, 생성된
			 * 파일명을 제가 원하는 파일명으로 변경하는 작업. if(!fileName.equals("")) { // fileName : 원본의 파일이름.
			 * // 원본이 업로드된 절대경로와 파일명를 구한다. String fullFileName = realFolder + "/" +
			 * fileName; // 파일 객체 생성 File f1 = new File(fullFileName); if(f1.exists()) { //
			 * 업로드된 파일명이 존재하면 Rename한다. //변경하고 싶은 파일명, 해당 경로와 해당 파일 그리고 확장자 포함. File newFile
			 * = new File(realFolder + "/" + uploadFileName); // 파일이름 변경.
			 * f1.renameTo(newFile); // rename... } } // 테스트 하기위해서 콘솔에 찍어 본 내용입니다. 참고.
...
//		member = dao.getMemberByNum(num, pageNum);
...
//		member.setContent(request.getParameter("content"));
		...
		member.setM_num(request.getParameter(""));
		member.setId(request.getParameter("id"));
		member.setPassword(request.getParameter("password"));
		member.setName(request.getParameter("name"));
		member.setGender(request.getParameter("gender"));
		member.setBirth(request.getParameter("birthyy"));
		member.setMail(request.getParameter("mail1"));
